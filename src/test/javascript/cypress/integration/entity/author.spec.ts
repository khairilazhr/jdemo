...
describe('Author e2e test', () => {
  ...
  it('should create an instance of Author', () => {
    ...
    cy.get(`[data-cy="name"]`).type('demand-driven', { force: true }).invoke('val').should('match', new RegExp('demand-driven'));

    cy.get(`[data-cy="age"]`).type('algorithm', { force: true }).invoke('val').should('match', new RegExp('algorithm'));

    cy.get(`[data-cy="birthDate"]`).type('2024-03-06').should('have.value', '2024-03-06');
