<template>
  <div>
    <h2 id="page-heading" data-cy="BookHeading">
      <span v-text="$t('jdemoApp.book.home.title')" id="book-heading">Books</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jdemoApp.book.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'BookCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-book">
            ...
            <span v-text="$t('jdemoApp.book.home.createLabel')"> Create a new Book </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && books && books.length === 0">
      <span v-text="$t('jdemoApp.book.home.notFound')">No books found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="books">
        ...
        <tbody>
          <tr v-for="book in books" :key="book.id" data-cy="entityTable">
            ...
            <td>{{ book.price }}</td>
            <td>
              <a v-if="book.cover" v-on:click="openFile(book.coverContentType, book.cover)" v-text="$t('entity.action.open')">open</a>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="jdemoApp.book.delete.question" data-cy="bookDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-book-heading" v-text="$t('jdemoApp.book.delete.question', { id: removeId })">
